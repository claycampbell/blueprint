...
3. Pushes to ECR (`connect2-app-dev`)
...
2. Tags with commit SHA and `latest`
3. Pushes to ECR (`connect2-app-staging`)
...
4. Pushes to ECR (`connect2-app-prod`)
...
```json
{
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Resource": [
        "arn:aws:s3:::connect2-app-terraform-state-*",
        "arn:aws:s3:::connect2-app-terraform-state-*/*"
      ]
    }
  ]
...
```
.github/workflows/
├── ci.yml              # Lint, test, build on all branches
...
| **Infrastructure PR** | Terraform, Docker, GitHub Actions, nginx.conf | `infra: add Redis cache` |
