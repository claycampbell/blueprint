...
router.get('/', async (req, res) => {
  try {
    ...
    let query = 'SELECT * FROM connect2.projects WHERE 1=1';
    const params = [];
...
router.post('/:id/documents', upload.single('file'), async (req, res) => {
  try {
    ...
    const s3Result = await uploadDocument(file.buffer, file.originalname, {
      ...
      uploadedBy: req.user?.id || 'system', // Assumes auth middleware sets req.user
    });
...
router.post('/:id/transition', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to transition project' });
  }
});
