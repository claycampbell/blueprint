...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(
      `${WINDMILL_URL}/api/w/${WINDMILL_WORKSPACE}/jobs/run/p/${path}`,
      {
        method: 'POST',
        headers: {
    ...
    console.log(`[Windmill] Job submitted: ${cleanJobId}`);
    ...
    for (let i = 0; i < maxAttempts; i++) {
      ...
      console.log(`[Windmill] Poll ${i + 1}: /completed status=${statusResponse.status}`);
      ...
      if (!statusResponse.ok) {
        ...
        console.log(`[Windmill] Poll ${i + 1}: /get status=${statusResponse.status}`);
      }

      if (statusResponse.ok) {
        const jobStatus = await statusResponse.json();
        console.log(`[Windmill] Poll ${i + 1}: success=${jobStatus.success}, has_result=${!!jobStatus.result}`);

        // Check if job has completed (has result field)
        if (jobStatus.success === true && jobStatus.result !== undefined) {
          console.log(`[Windmill] Job completed successfully`);
          return NextResponse.json(jobStatus.result);
        }

        if (jobStatus.success === false) {
          console.log(`[Windmill] Job failed`);
          return NextResponse.json(
    ...
    console.log(`[Windmill] Job timed out after ${maxAttempts} attempts`);
